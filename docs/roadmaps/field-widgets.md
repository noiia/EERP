...
| boolean/picture | Field value = "image exists". Upload/replace/delete through the picture service; widget shows a ringed thumbnail once uploaded, or an equally-ringed placeholder box beforehand (never a bare Upload button floating with nothing to anchor it). Box size (both states) is `widgetOptions: { width, height }` in pixels — declared by the module like `stars`' `max`, defaulting to 160×96 when unset or not a number. |
