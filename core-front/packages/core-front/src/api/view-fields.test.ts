...
describe('effectiveViewFields', () => {
  it('falls back to the module default when nothing is overridden', () => {
    ...
    ).toEqual({ kanbanStatusField: 'status', calendarDateField: 'due_date', enableGraphs: true })
  })
  ...
  it("a non-null Settings override wins over the module's default, field by field", () => {
    ...
    ).toEqual({ kanbanStatusField: 'priority', calendarDateField: 'due_date', enableGraphs: false })
  })
  ...
  it('an override left null (never touched) still inherits the module default', () => {
    ...
    ).toEqual({ kanbanStatusField: 'status', calendarDateField: 'due_date', enableGraphs: false })
  })
})
...
describe('availableDisplayModes', () => {
  it('list is always available; kanban/calendar/graph follow the effective config', () => {
    expect(
      availableDisplayModes({ kanbanStatusField: null, calendarDateField: null, enableGraphs: false }),
    ).toEqual({ list: true, kanban: false, calendar: false, graph: false })
    expect(
      availableDisplayModes({ kanbanStatusField: 'status', calendarDateField: 'due_date', enableGraphs: true }),
