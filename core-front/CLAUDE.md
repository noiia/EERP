...
**Active flag:** discovery respects `module.json` `active`: a module with `active: false` contributes neither views nor translations — the backend doesn't serve a deactivated module, so compiling its frontend would produce dead routes and orphan strings. A missing `active` counts as active (only the explicit opt-out disables). Deactivating a module is therefore one flag flip + rebuild on both sides.
