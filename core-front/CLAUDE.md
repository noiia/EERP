...
| Form chatter panel | Every `viewType: 'form'` route renders a per-record activity feed beside/below the form — `renderers.tsx`'s `FormRenderer` wraps its existing form `Box` and a new `ChatterPanel` (`chatter-panel.tsx`) in a flex container: `row` (panel to the RIGHT) at/above `tokens.ts`'s `layout.chatterBreakpoint` (1500px, a plain viewport media query — this wrapper is always the whole form page, never re-embedded in a narrower container the way `LayoutForm` is), `column` (panel full-width BELOW) under it. The panel is user-resizable via a plain `mousedown`/`mousemove`/`mouseup` drag handle on its own left edge (no library — a single fixed-axis drag didn't earn one; `react-resizable` is only ever react-grid-layout's own internal, untyped-for-app-code peer, see the root layout's CSS-only import of it), width clamped to `chatterWidthMin`/`Max` and persisted per-user (not per-entity) in `useUiStore.chatterWidth`. Data path: `ChatterOps` (`chatter-ops.tsx`) is `NotebookOps`'s exact shape minus update/remove — `{ list, create }`, `useChatterOps()` returning `null` with no provider mounted (renders nothing, same inert posture every Ops context takes) — bound Server Actions (`apps/shell/src/lib/chatter-actions.ts`) the root layout provides once via `ChatterOpsProvider`. Two kinds of entry, both posted through the SAME `ChatterOps.create` and rendered identically as `"<author> : <body>"` (log entries styled at reduced opacity): `kind: 'message'` from the panel's own composer (a `TextField` + Send button, hidden behind the picture widgets' "Available once the record has been saved." hint until the record has an id), and `kind: 'log'` — `FormRenderer`'s own post-save summary. On every successful commit of an ALREADY-EXISTING record (never a first-time create — "edition", not "creation"), `summarizeFieldChanges` diffs the record's field values from just before this save against what Go just confirmed (virtual o2m/m2m relations skipped, a no-op round-trip back to the same value skipped) and, if anything changed, fires `ChatterOps.create(entity, id, 'log', "Field: old → new; ...")` — fire-and-forget (`.catch(() => {})`), since a log-post failure must never surface as a save error once the record itself already saved. Go's `internal/chatter` never diffs anything itself; it only stores whatever `kind`/`body` the caller (composer or form save) posts — see `core/CLAUDE.md`'s `internal/chatter` bullet for the backend shape and its accepted "client-declared kind" simplification. |
