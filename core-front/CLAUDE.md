...
| Field widgets | `FieldDescriptor.widget` picks presentation per data type (`FIELD_WIDGETS` matrix, first entry = default): text `simple`/`long`/`phone` · number `float`/`int`/`percent`/`stars`/`phone` · boolean `switch` · date/relation stock. `widgetOptions` tunes it (`{ max: 5 }`, `{ decimals: 0 }`, `{ base: 'percent' }`) — JSON only. Invalid pairs fail at `registry.register`, naming module/route/field. Numeric widgets format exclusively through `useNumberFormat()` (workspace separators — `app_settings` `format.number`, edited in Settings → Formats behind `settings:format:write`, mirrored client-side by `useFormatStore` and seeded by the preferences load). See `docs/roadmaps/field-widgets.md`. |
