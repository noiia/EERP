...
// Go's claims: { sub: userId, tenant: tenantId, roles: [], groups: [], permissions: [], exp }.
// The permissions claim carries the role-derived codes that light up <Can> and the Create
// buttons; it refreshes with the token (grant changes apply on the next issue), and it
// is UI gating only — Go re-authorizes every call from the database. groups is the same
// posture for field-level visibility (core/orm's WithFieldGroups): Go's generic CRUD
// already omits a gated field's key server-side, so this is a UX mirror, not a gate.
