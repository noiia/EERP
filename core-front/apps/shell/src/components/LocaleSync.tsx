...
// Applies the server-owned preferences fetched by the root layout to the client
// mirrors on load: the language (per-user choice + workspace default → useI18nStore)
// and the workspace number format (→ useFormatStore). The server is the source of
// truth — the stores are client mirrors, kept for instant rendering and as the
// fallback while logged out or when the read failed (preferences === null). A locale
// arriving from the server is auto-enabled so it is selectable in the settings UIs
// even on a browser that never "added" it.
export function LocaleSync({ preferences }: { preferences: LocalePreferences | null }) {
  useEffect(() => {
    ...
    // null format = the workspace never set one: keep the mirror (built-in default
    // or the last synced value) instead of clobbering it.
    if (preferences.number_format) {
      useFormatStore.getState().setNumberFormat({
