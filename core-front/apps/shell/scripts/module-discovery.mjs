...
/**
 * Render the CLIENT twin of the manifest: bare side-effect imports of every view
 * file, nothing else. Views files register behaviors (registerFieldFunction /
 * registerOnChange) at import time, and those registries live per-bundle — the
 * server manifest above only populates the SERVER instance (it imports the
 * server-only barrel, so it can never enter the browser bundle). A client
 * component (ModulesInit) imports this file so the registrations also evaluate
 * in the browser, where the form store validates compute names. Mirrors the
 * generated-translations / I18nInit pattern.
 */
export function renderClientManifest(discovered, fromDir) {
  const lines = [
    '// AUTO-GENERATED by scripts/generate-modules.mjs — do not edit.',
    '// Side-effect imports only: evaluates every module views file in the BROWSER',
    '// bundle so their registerFieldFunction/registerOnChange calls populate the',
    '// client-side behavior registry (imported by ModulesInit). Gitignored.',
  ]
  for (const module of discovered) {
    for (const view of module.views) {
      lines.push(`import '${toImportSpecifier(fromDir, view.sourceFile)}'`)
    }
  }
  lines.push('')
  return lines.join('\n')
