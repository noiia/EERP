...
export const userFormDescriptor: ViewDescriptor<AdminRecord> = {
  ...
  fields: [
    ...
    {
      // Virtual many2many over the generic-CRUD-registered user_roles
      // junction (core/internal/auth.UserRoles) — an Active Directory-style
      // group membership, a user can hold several. Stripped from the PUT
      // body and written instead through its own endpoint, same posture as
      // roleFormDescriptor's `belongs` below.
      name: 'role',
      label: 'Roles',
      type: 'relation',
      relation: {
