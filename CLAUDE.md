...
- `internal/auth/` — authentication (login/refresh/logout, JWT, permission DSL) plus the dedicated users/roles administration endpoints (`GET|POST /api/v1/users` + `GET|PUT /api/v1/users/:id`, same shape for `/api/v1/roles`). The auth tables are excluded from the generic CRUD surface on purpose, so these tenant-pinned, field-whitelisting handlers (profile fields — email, username, name/surname/display_name/job_title/phone, the 7-column address composite, password — for users; name/description/technical_name for roles) are the only HTTP path to them; the permission middleware derives `users:users:*` / `roles:roles:*` from the routes. A POSTed user with no password is created **locked** — its password hash comes from discarded random bytes; a blank password on an update leaves the existing credential unchanged. `core/modules/auth` is a regular `app_mode: true` module (display name "Accounts") backing these with a frontend app — like any other module, not a bespoke Settings page — at `/accounts` (`core/modules/auth/views/`); converting it only changed how the routes/landing tile register, not this HTTP contract. The User form's `role` many2many field assigns roles to a user (several allowed, acting as Active-Directory-style groups) over `UserRoles` — like `RoleBelongs`, it rides the generic CRUD surface (table `user_roles`, a hand-written unique index on `(user_id, role_id)` since struct tags can't express one) so the tags widget needs no bespoke endpoint; `UserRepository.FindRoleNames`/`FindGroups` filter its `deleted_at` themselves now that it's soft-deletable. The Role form's own first notebook page ("Views") is a `RoleViewPermission` table riding the generic CRUD surface — one row per entity the role has any rights on (a many2one, picked from the live catalog at `GET /api/v1/views`, `internal/settings.GetViewCatalog` — every route prefix currently registered on the generic CRUD surface, i.e. this deployment's compiled-in modules) with a `rights` many2many (`RoleViewPermissionRight` → `AccountRoleTypes`: deny/read/write/delete) — **data model and UI only**, not yet consulted by the `module:resource:action` permission check above, which stays the sole enforcement path.
