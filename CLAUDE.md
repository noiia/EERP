...
- `internal/settings/` — tenant-scoped `app_settings` key/value store (kept off the generic CRUD surface) plus the dedicated handlers behind it: `GET/PUT /api/v1/me/preferences` (self-service, JWT-scoped — e.g. the user's `preferred_locale`), `PUT /api/v1/settings/i18n` (workspace default language, key `i18n.default_locale`, permission `settings:i18n:write`), `PUT /api/v1/settings/format` (workspace number format, key `format.number`, permission `settings:format:write`), `GET/PUT /api/v1/settings/views/:entity/fields` (which field, if any, is that entity's Kanban status field / Calendar date field, key `views.<entity>.fields`, permissions `settings:views:read`/`settings:views:write` — see `docs/roadmaps/list-view-modes.md` and `docs/adr/ADR-006-runtime-configurable-view-fields.md`), and `GET/PUT /api/v1/settings/views/:entity/graph` (entity's Graph mode tile layout, key `views.<entity>.graph`, same permissions — an unconfigured entity reads as an empty canvas, not a 404; the backend validates each tile's shape — id, non-negative/non-zero geometry, closed type set, no duplicate ids — but never its opaque `config`), `GET/PUT /api/v1/settings/integrations/osm` (the workspace's OpenStreetMap/Nominatim connector config — `{enabled, base_url, user_agent}`, key `integrations.osm`, permissions `settings:integrations:read`/`settings:integrations:write` — an unconfigured/disabled connector reads as the zero value, not a 404; consumed by `core-front`'s `AddressWidget` via a Next BFF proxy route, never called from the browser directly, so the connector's own config — and any future API key it might need — never reaches client JS), and `GET/PUT /api/v1/settings/tax` (the workspace's tax pricing mode — `{price_mode: "tax_excluded"|"tax_included"}`, key `tax.price_mode`, permissions `settings:tax:read`/`settings:tax:write` — absent reads as `tax_excluded`, the original behavior; `ResolveTaxIncluded` is exported so `sale`/`propertymanagement`'s own tax computation can resolve the same effective value without going through HTTP), and `GET/PUT /api/v1/settings/units` (the workspace's default unit system — `{system: "metric"|"imperial"}`, key `units.system`, permissions `settings:units:read`/`settings:units:write` — absent reads as `metric`; `ResolveUnitSystem` is exported, same split as `ResolveTaxIncluded`, so `propertymanagement`'s own property Create override can resolve it without HTTP). `tax_excluded` (the default) computes a line's tax on top of its base price (`12€ + 20% tax = 14.40€`); `tax_included` treats the base price as already tax-inclusive and back-derives the pre-tax subtotal (`12€ including 20% tax stays 12€, of which 10€ is pre-tax`) — applies uniformly to a `SaleTax`'s percentage and fixed kinds. `sale.SaleLine`/`propertymanagement.PropertyManagementBillingLine`/`PropertyManagementRentReceiptLine` each carry a persisted `Subtotal` column (the line's own tax-excluded contribution) alongside `Total`, computed once at write time (`computeLineTotal`/`computeBillingLineTotal`) so invoice/receipt rollups (`sumLines`/`recomputeReceiptTotals`) can sum it directly rather than re-deriving it from `UnitPrice`, which no longer safely stands in for the tax-excluded figure once prices can be tax-inclusive.
- `internal/auth/` — authentication (login/refresh/logout, JWT, permission DSL) plus the dedicated users/roles administration endpoints (`GET|POST /api/v1/users` + `GET|PUT /api/v1/users/:id`, same shape for `/api/v1/roles`). The auth tables are excluded from the generic CRUD surface on purpose, so these tenant-pinned, field-whitelisting handlers (profile fields — email, username, name/surname/display_name/job_title/phone, the 7-column address composite, password — for users; name/description/technical_name for roles) are the only HTTP path to them; the permission middleware derives `users:users:*` / `roles:roles:*` from the routes. A POSTed user with no password is created **locked** — its password hash comes from discarded random bytes; a blank password on an update leaves the existing credential unchanged. They back the frontend's Settings → Users pages. `core/modules/auth` also carries a `module.json` (`app_mode: false`, no frontend `static_files` — same shape as `company`'s) purely so it's manageable from the App Store (`Settings → Apps`: activate/deactivate/reload) like any other module; it registers no views of its own, so Settings → Users stays the only UI for these entities. The User form's `role` many2many field assigns roles to a user (several allowed, acting as Active-Directory-style groups) over `UserRoles` — like `RoleBelongs`, it rides the generic CRUD surface (table `user_roles`, a hand-written unique index on `(user_id, role_id)` since struct tags can't express one) so the tags widget needs no bespoke endpoint; `UserRepository.FindRoleNames`/`FindGroups` filter its `deleted_at` themselves now that it's soft-deletable. The Role form's own first notebook page ("Views") is a `RoleViewPermission` table riding the generic CRUD surface — one row per entity the role has any rights on (a many2one, picked from the live catalog at `GET /api/v1/views`, `internal/settings.GetViewCatalog` — every route prefix currently registered on the generic CRUD surface, i.e. this deployment's compiled-in modules) with a `rights` many2many (`RoleViewPermissionRight` → `AccountRoleTypes`: deny/read/write/delete) — **data model and UI only**, not yet consulted by the `module:resource:action` permission check above, which stays the sole enforcement path.
...
- `internal/reports/` — PDF report generation (`docs/adr/ADR-010-pdf-report-generation.md`, `docs/roadmaps/pdf-reports.md`): a module-declared `ReportDescriptor` renders to a print-only HTML route the frontend serves, which an EXTERNAL Chromium-backed renderer — `tools/pdf-service/`, a separate Go binary/container, never code running inside `core` — screenshots to PDF. `POST /api/v1/reports/:name/:id/pdf` (`GeneratePDF`, deliberately NOT behind the permission middleware — see its own doc comment) triggers a render and stores the bytes through the SAME S3-compatible object store `internal/pictures`/`internal/attachments` already use; `GET /api/v1/reports/pdf` (permission-derived) downloads it. `reports.PDFRenderer` is an interface with two implementations picked by config at boot (`core/cmd/app/main.go`): `NewHTTPPDFRenderer(pdf_service_url)` calls pdf-service directly over HTTP (zero extra infra), or — once `nats_url` is set — `NewNATSPDFRenderer` publishes the render request over NATS request-reply on subject `reports.render` and awaits the reply on the same subject; NATS wins when both are configured, since setting it names real intent to run pdf-service as a scalable pool of stateless worker replicas (`tools/pdf-service/natsworker.go`'s `QueueSubscribe("reports.render", "pdf-workers", ...)` — a competing-consumer queue group, so replicas load-balance renders with zero extra code on either side). The whole feature mounts only when S3, a renderer target (`pdf_service_url` or `nats_url`), and `frontend_base_url` are all configured — absent any one, report generation simply isn't mounted, same posture as pictures' own `s3_*` gate. See "Deployment topology" below for how pdf-service sits relative to the gateway.
...
- `internal/presence/` — live user status (online/absent/offline/busy/do_not_disturb) over a WebSocket, plus a REST snapshot for first load (`docs/adr/ADR-019-user-presence-websocket.md`). One `UserPresence` row per user (`manual_status` nullable — only ever `"busy"`/`"do_not_disturb"`, absent/offline are never manually selectable — plus `connected`/`last_seen`), off the generic CRUD surface like `internal/chatter`/`internal/notebook`; the dedicated handler resolves the caller's own row from the JWT identity, no `:id` param. `repository.go`'s writes go through `orm.Repository.Upsert` (the natural-key `(tenant_id, user_id)` upsert primitive, ORM section above) with an explicit SET fragment per call so a `SetConnected` never clobbers a concurrently-set `manual_status` and vice versa — reads go through the same repository's plain `FindAll`. `status.go`'s `Effective(manual, connected, lastSeen)` is the single place the five colors get decided (a manual busy/DND override wins even while disconnected; otherwise online while connected, absent up to 30 minutes disconnected, then offline) — used identically for the REST read, the WS snapshot, every broadcast, and the sweep. One route, `GET|PUT /api/v1/presence` — GET dispatches to either the plain JSON snapshot or a WebSocket upgrade (`websocket.IsWebSocketUpgrade`) so both resolve to the SAME `presence:presence:read` permission instead of needing separate grants for a `/ws` sub-path; PUT sets/clears the caller's own manual override. Mounted behind `middleware.JWTOrCookieMiddleware` (not the plain `jwtMw` every other route uses) — a native WebSocket handshake can't carry a custom `Authorization` header, so this one route group falls back to reading the SAME access-token JWT out of the Next BFF's httpOnly session cookie when there's no Bearer header, the one deliberate exception to "the browser never calls Go directly" (see the ADR). `hub.go`'s `Hub` is an in-memory, single-Go-instance connection registry (`compose.yml` runs one `core-back` replica; `internal/reports`' NATS queue-group pattern is the documented upgrade path if that ever changes) that fans a tenant's status updates out to every open connection. `sweeper.go`'s `SweepAbsentToOffline` runs off a plain one-minute ticker in `main.go` (mirroring `sale.ExpireOverdueQuotes`'s own ticker, not `internal/cron`'s heavier `Scheduler` — presence has exactly one fixed job) to push the absent→offline transition, since no request happens at the exact instant the 30 minutes elapse.
- `internal/graphfield/` — chart-only **calculated fields** for the Graph view (`docs/adr/ADR-020-graph-calculated-fields.md`): a named infix formula (`rent_price - loan_amount * 0.5` — field keys, other calc keys, number literals, `+ - * /`, parentheses) a user creates from the Graph view's edit mode and any tile type (xy/bar/pie/stat/list) can then plot like a real column. `GraphField` table (`tenant_id`/`entity`/`field_key`/`label`/`formula`/`roles`), off the generic CRUD surface, unique `(tenant_id, entity, field_key)` hand-written in `modules/graphfield`. Dedicated tenant-pinned routes `GET|POST /api/v1/graph_fields` (query `entity`) + `DELETE /api/v1/graph_fields/:id` (permissions `graph_fields:graph_fields:*` derived from the route). `roles` (comma-joined role technical names, empty = everyone) is enforced on list AND delete: a caller whose roles/groups don't intersect never sees the field, and a delete on it 404s. Delete is a real `HardDelete` (row + index entries gone, key reusable). The formula is validated by charset only and **evaluated client-side** (`core-front/.../graph-aggregate.ts` `evalFormula`, hand-rolled parser, never `eval`) over the records the tile already fetched, so it can never see a column ADR-013 gating already stripped; an unknown identifier reads 0, which is exactly how a deleted field "becomes 0" in every tile that referenced it (`withCalculatedFields` zero-fills `calc_*` keys found in tile configs). An xy/bar tile config takes `yFields: string[]` (several lines on one chart) beside the legacy `yField`.
...
- `internal/dbmanage/` — the unauthenticated, master-key-gated database manager behind `/database/management` (`docs/adr/ADR-021-database-management.md`): list/create/switch/delete the Postgres databases on this server, and extract/restore a database (a `pg_dump` plus, optionally, its `picture`/`attachment` S3 content) as a portable zip. Mounted on its own `/api/v1/database-management/*` Echo group with **no** `jwtMw`/`permMw` — every handler instead calls `RequireMasterKey` itself, comparing an `X-Master-Key` header against the SAME `Config.MasterPassword` that signs every JWT (one secret; this page is an alternate full-compromise path if it leaks — an accepted trade-off, not an oversight), rate-limited like `/api/v1/auth`. **Switching is a true live hot-swap, no restart**: `db.DB.SwapPool` (`core/orm/pool/db/db.go`, `pool` is an `atomic.Pointer[pgxpool.Pool]`) repoints the SAME `*orm.DB` every `Repository[T]`/handler/background ticker already holds a reference to, then `SwitchTo` re-runs the LIVE `module.Registry.Boot` (not a throwaway one, so `ActiveGateMiddleware`'s state stays correct), reseeds the default admin, force-disconnects every presence websocket (`presence.Hub.CloseAll`), and best-effort persists `db_name` into the on-disk config (silently degrades under Docker's typically read-only config mount — the live switch already happened regardless). This is also what made `internal/module/go_module.go`'s `loadGoModule` idempotency bug worth fixing (see its own doc comment): a process-global in-memory diff, not real DB state, decided whether to (re-)create Go-module tables, so a second in-process `Boot()` — exactly what switch/create need — silently created nothing on an empty target before that fix. S3 extraction is **manifest-driven**: `BuildManifest` queries the database being extracted for every `picture`/`attachment` `object_key`, scoping the zip to exactly that database's own objects out of a bucket potentially shared by several EERP databases — generated report PDFs are never included (`internal/reports` persists no DB row for its own key; always regeneratable from the dump's own invoice/quote/receipt data). `pg_dump`/`pg_restore` are shelled out to (PGDG-installed in `core/Dockerfile`, pinned to the server's Postgres major version) rather than reimplemented. Frontend: `core-front/apps/shell/app/database/management/page.tsx` is a plain client component outside `requireAuth()`, calling this API directly with no BFF — the same "browser calls Go directly for a good reason" exception ADR-019 established for Presence, here because of large binary transfer with no session involved.
...
// Upsert — insert, or apply setFragment on a conflict against a natural key
// (must be backed by a real unique index — see "Struct tags have no
// unique-constraint support" below). setFragment is a raw "col = EXCLUDED.col"
// SQL fragment, not auto-derived from every column, since a natural-key
// upsert commonly writes only SOME columns and must leave the rest untouched.
upserted, _ := presence.Upsert(ctx, row,
    []string{"tenant_id", "user_id"},
    "connected = EXCLUDED.connected, updated_at = now()")
// setFragment == "" means ON CONFLICT ... DO NOTHING — idempotent,
// deterministic-id seeding (internal/auth/seed.go). Upsert transparently
// re-reads the existing row by conflictColumns in that case, since Postgres's
// RETURNING gives back nothing for a skipped DO NOTHING.
role, _ := roles.Upsert(ctx, Roles{BaseModel: model.BaseModel{ID: fixedID}, ...}, []string{"id"}, "")

// Partial/conditional update — Update()'s full-row-replace-by-PK contract
// doesn't fit "bump only some columns" or "match on something other than
// the PK". UpdateQuery() is the write-side counterpart to Query()'s
// read escape hatch.
n, _ := orders.UpdateQuery().
    Set("status", "shipped").
...
results, _ := query.Select[Order](orders.Meta()).
    ...
    Where(orm.In("customer_id", customerIDs)). // "col = ANY($1)", not a hand-built IN (…) placeholder list
...
Struct tags use `db:"column_name"`, with optional `,pk` and `,softdelete` modifiers. Metadata is resolved once at `MustRepo` construction via `core/orm/internal/cache` — zero reflection at query time. `Create`/`Upsert` insert the PK column too when it's explicitly set (same zero-value-skip rule already applied to `created_at`/`updated_at` — a DB-side `DEFAULT gen_random_uuid()` fills it in otherwise), so a caller-chosen id (deterministic idempotent seeding, `internal/auth/seed.go`) inserts as given rather than being silently dropped in favor of a fresh one.
...
**Deployment topology:** every `compose.yml` service and how they actually reach each other — the browser only ever talks to the gateway, never directly to `core-front` or `core-back`, and `pdf-service` is reachable from neither the gateway nor the browser at all (see `internal/reports/` above).
